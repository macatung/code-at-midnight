...
const desktopRoot = fs.existsSync(path.join(root, 'desktop'))
  ? path.join(root, 'desktop')
  : path.resolve(root, '../task-hub/apps/desktop');

const mainSource = fs.readFileSync(path.join(desktopRoot, 'electron/main.ts'), 'utf8');
const uiSource = fs.readFileSync(path.join(desktopRoot, 'src/components/AgentConsoleModal.vue'), 'utf8');
...
describe('Desktop agent execution workspace', () => {
  ...
  it('exposes preflight and isolated worktree IPC actions', () => {
    ...
    expect(mainSource).toContain('human approval');
  });
  ...
  it('requires a structured handoff after execution', () => {
    ...
    expect(uiSource).toContain('Submit Handoff');
  });
});
