...
const { activePhase, activePhaseId, isTimeTravelActive, formattedTime } = useTimeCycle();
const {
  currentReaction,
  isIdleSleeping,
  totalHops,
...
// Draggable Position (Fixed coordinate system with bounds protection)
const posX = ref(0);
const posY = ref(0);
let startX = 0;
let startY = 0;
...
const onPointerDown = (e: PointerEvent) => {
  // Only drag from mascot avatar or header handle
...
const onPointerMove = (e: PointerEvent) => {
  ...
  // Constrain inside viewport with margins
  const maxX = window.innerWidth - 280;
  const maxY = window.innerHeight - 320;
  
  posX.value = Math.max(-maxX + 40, Math.min(40, initialX + deltaX));
  posY.value = Math.max(-maxY + 40, Math.min(40, initialY + deltaY));
};
...
<template>
  <!-- Floating Screen Pet Container -->
  <aside
    class="fixed bottom-5 right-5 z-40 select-none transition-transform duration-150 ease-out"
    :style="{
  ...
  >
    ...
    <div
      v-if="isMinimized"
      class="group flex items-center gap-2 px-3 py-2 rounded-2xl bg-midnight-950/90 border border-slate-700/80 backdrop-blur-xl shadow-2xl cursor-pointer hover:border-phantom-mint hover:scale-105 transition-all duration-200"
      @click="toggleMinimize"
      title="Mở rộng Ma Cà Tưng Companion"
    >
      <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-900 border border-slate-700 text-base shadow-inner animate-bounce">
        🧛‍♂️
      </span>
      <div class="flex flex-col pr-1">
        <span class="text-xs font-mono font-bold text-phantom-mint flex items-center gap-1.5">
          <span>Ma Cà Tưng</span>
          <span class="inline-block w-1.5 h-1.5 rounded-full bg-phantom-mint animate-pulse" />
        </span>
    ...
    <div
      v-else
      class="flex flex-col items-end gap-2 max-w-[320px] w-full"
    >
      ...
      <div
        class="relative px-3.5 py-2.5 rounded-2xl bg-midnight-950/95 border text-xs font-sans font-medium text-slate-200 shadow-2xl backdrop-blur-xl transition-all duration-300 pointer-events-auto"
        :style="{
          borderColor: activePhase.accentBorder,
          boxShadow: `0 8px 30px -4px ${activePhase.accentGlow}`
        }"
      >
      ...
      <div
        class="flex items-center gap-3 p-3 rounded-2xl bg-midnight-950/90 border border-slate-800/90 backdrop-blur-2xl shadow-2xl hover:border-slate-700/80 transition-all duration-300"
        :class="{ 'ring-2 ring-phantom-mint/30': isDragging }"
      >
        <!-- Interactive Avatar (Click to Hop / Drag to Move) -->
        <div
          class="relative cursor-grab active:cursor-grabbing flex flex-col items-center justify-center group/avatar"
        ...
        <div class="flex flex-col gap-1.5 border-l border-slate-800/80 pl-3">
          <div class="flex items-center justify-between gap-1 text-[10px] font-mono text-slate-400">
            ...
            <button
              @click="toggleMinimize"
              class="text-slate-500 hover:text-slate-300 px-1 hover:bg-slate-800 rounded transition-colors"
              title="Thu nhỏ"
            >
              ▼
            </button>
          ...
          <div class="grid grid-cols-2 gap-1.5">
            ...
            <button
              @click="feedCoffee"
              class="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900/80 hover:bg-amber-950/40 border border-slate-800 hover:border-amber-500/50 text-[11px] font-medium text-slate-300 hover:text-amber-300 transition-all duration-150 active:scale-95"
            ...
            <button
              @click="applyTalisman"
              class="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900/80 hover:bg-emerald-950/40 border border-slate-800 hover:border-emerald-500/50 text-[11px] font-medium text-slate-300 hover:text-phantom-mint transition-all duration-150 active:scale-95"
              title="Yểm bùa 0-Bug"
            >
              <span>📜</span>
              <span>Dán bùa</span>
            ...
            <button
              @click="petMascot"
              class="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900/80 hover:bg-pink-950/40 border border-slate-800 hover:border-pink-500/50 text-[11px] font-medium text-slate-300 hover:text-pink-300 transition-all duration-150 active:scale-95"
            ...
            <button
              ...
              :class="[
                'flex items-center gap-1 px-2 py-1 rounded-lg border text-[11px] font-medium transition-all duration-150 active:scale-95',
                showTimeTraveler
                  ? 'bg-amber-500/20 text-amber-300 border-amber-500/60 shadow-sm'
                  : 'bg-slate-900/80 hover:bg-cyan-950/40 border-slate-800 hover:border-cyan-500/50 text-slate-300 hover:text-cyan-300'
              ]"
