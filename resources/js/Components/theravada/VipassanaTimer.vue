  ...
  <div class="w-full max-w-4xl mx-auto p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-stone-900/95 via-stone-950/90 to-stone-900/95 border border-amber-500/30 shadow-2xl backdrop-blur-xl font-serif text-stone-100 relative overflow-hidden">
    ...
    <div class="relative z-10 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-amber-500/20 pb-5 text-left">
        ...
        <div class="shrink-0 px-4 py-2.5 rounded-2xl bg-stone-950/80 border border-stone-800 flex items-center gap-2.5 text-xs text-stone-300">
      ...
      <div class="flex flex-col items-center justify-center py-6 sm:py-10">
        ...
        <div class="relative flex items-center justify-center">
          ...
          <div
            ...
            :class="[
              isRunning && !isPaused
                ? breathingPhase === 'inhale' ? 'w-80 h-80 opacity-90' : 'w-56 h-56 opacity-30'
                : 'w-64 h-64 opacity-20'
            ]"
          />

          <!-- SVG Progress Circle -->
          <svg class="w-64 h-64 sm:w-72 sm:h-72 -rotate-90" viewBox="0 0 240 240">
            <!-- Background track -->
          ...
          <div class="absolute flex flex-col items-center justify-center text-center space-y-1">
            <span class="text-4xl sm:text-5xl font-mono font-bold text-amber-100 tracking-wider">
              {{ formattedTime }}
            </span>

            <!-- Breathing Status -->
            <div v-if="isRunning && !isPaused" class="transition-all duration-700">
              <span
                class="inline-block px-3 py-1 rounded-full text-xs font-serif font-bold transition-all duration-1000"
                :class="breathingPhase === 'inhale' ? 'bg-amber-500/25 text-amber-200 border border-amber-400' : 'bg-stone-800 text-stone-300'"
              >
                {{ breathingPhase === 'inhale' ? '🌸 Hít Vào — Tâm Tĩnh Lặng' : '🍃 Thở Ra — Miệng Mỉm Cười' }}
              </span>
            </div>
            <div v-else class="text-xs font-serif text-stone-400">
        ...
        <div class="mt-8 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          <span class="text-xs text-stone-400 w-full sm:w-auto text-center sm:text-left mb-2 sm:mb-0">
            Thời lượng:
          </span>
          <button
            ...
            :disabled="isRunning"
            :class="[
              'px-3.5 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-serif transition-all cursor-pointer border',
      ...
      <!-- Action Controls Bar -->
      <div class="flex flex-wrap items-center justify-center gap-4 pt-5 border-t border-stone-800">
        <!-- Start Button -->
        <button
          v-if="!isRunning"
          @click="startTimer"
          class="flex items-center gap-2 px-8 py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-stone-950 font-serif font-bold text-sm sm:text-base shadow-xl transition-all hover:scale-105 active:scale-95 cursor-pointer"
        >
          <span>🔔</span>
        ...
        <button
          ...
          @click="pauseTimer"
          class="flex items-center gap-2 px-6 py-3 rounded-2xl bg-stone-900 border border-stone-700 text-stone-200 hover:text-amber-300 font-serif text-sm font-bold transition-all cursor-pointer"
        >
          <span>⏸️</span>
          <span>Tạm Dừng</span>
        </button>
        ...
        <button
          v-if="isRunning && isPaused"
          @click="resumeTimer"
          class="flex items-center gap-2 px-6 py-3 rounded-2xl bg-amber-500 text-stone-950 font-serif text-sm font-bold shadow-lg hover:bg-amber-400 transition-all cursor-pointer"
        >
          <span>▶️</span>
          <span>Tiếp Tục</span>
        </button>
        ...
        <button
          v-if="isRunning"
          @click="resetTimer"
          class="flex items-center gap-1.5 px-5 py-3 rounded-2xl bg-stone-900 border border-rose-500/30 text-rose-300 hover:bg-rose-950/40 text-xs sm:text-sm font-serif font-bold transition-all cursor-pointer"
        >
          <span>⏹️</span>
