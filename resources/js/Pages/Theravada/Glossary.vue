...
<template>
  ...
  >
    <div class="max-w-5xl mx-auto py-6 sm:py-10">
      <!-- Breadcrumb -->
      <nav class="flex items-center gap-2 text-xs font-serif text-stone-300 mb-6">
      ...
      <header class="mb-10 text-left border-b border-stone-800 pb-8">
        ...
        <p class="text-xs sm:text-sm text-stone-300 font-serif mt-2 max-w-3xl leading-relaxed">
        ...
        <div class="mt-6 relative max-w-xl">
          <input
            ...
            class="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-stone-900 border-2 border-stone-700 text-sm sm:text-base text-stone-100 placeholder-stone-400 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/40 shadow-inner"
          />
          <span class="absolute left-4 top-4 text-amber-400 text-sm">🔍</span>
        </div>
      ...
      <div class="space-y-4">
        <div
          ...
          :key="item.term"
          class="p-6 rounded-3xl bg-stone-900/90 border border-stone-800 hover:border-amber-500/60 transition-all group cursor-pointer shadow-md text-left"
          @click="strikeBell"
        >
          <div class="flex flex-wrap items-baseline justify-between gap-2 mb-3 pb-2 border-b border-stone-800/80">
            <div class="flex items-center gap-3">
              <h3 class="text-xl sm:text-2xl font-serif font-bold text-amber-300 group-hover:text-amber-200">
                {{ item.term }}
              </h3>
              <span v-if="item.vietnamese && item.vietnamese !== item.term" class="text-base font-serif italic text-amber-100">
                ({{ item.vietnamese }})
              </span>
            </div>
          ...
          <p class="text-sm sm:text-base text-stone-200 font-serif leading-relaxed">
            {{ item.definition }}
          </p>
        </div>

        <div v-if="filteredList.length === 0" class="py-16 text-center text-stone-400 font-serif">
