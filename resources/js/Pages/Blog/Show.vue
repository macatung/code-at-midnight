...
const formattedContent = computed(() => {
  ...
  // 2.5 Sanitize LaTeX Math & Flow arrows
  content = content
    .replace(/\$\$\s*([\s\S]*?)\s*\$\$/g, (_match, inner) => {
      const cleanFlow = inner
        ...
        .trim();
      return `\n\n<div class="my-4 p-4 rounded-xl border border-slate-700 bg-slate-900/90 text-emerald-400 font-mono text-sm sm:text-base flex items-center justify-center text-center font-medium shadow-sm overflow-x-auto"><span>${cleanFlow}</span></div>\n\n`;
    })
    .replace(/\$\\(long)?rightarrow\$/g, '➔')
    .replace(/\\(long)?rightarrow/g, '➔')
    .replace(/\\text\{([^}]+)\}/g, '$1');
...
<template>
  ...
  <div class="min-h-screen bg-midnight-950 text-slate-100 flex flex-col overflow-x-hidden bg-grid-pattern">
    ...
    <main class="relative z-10 flex-1 w-full max-w-4xl mx-auto px-4 py-10 sm:px-6 sm:py-14">
      <nav class="mb-8 flex items-center gap-2 text-xs font-mono text-slate-400" aria-label="Breadcrumb">
      ...
      <header class="mb-8 border-b border-white/10 pb-8">
        ...
        <div class="mt-5 flex items-center gap-3 text-xs font-mono text-slate-400">
          <span>{{ article.published_at }}</span>
          <span>·</span>
          <span class="text-phantom-mint">{{ article.reading_time_min }} {{ t('common.minutes') }}</span>
        </div>
      ...
      <article class="rounded-3xl bg-white p-6 text-slate-800 shadow-2xl sm:p-10 lg:p-12">
      ...
      <div class="mt-8 flex items-center justify-between">
        <Link href="/blog" class="text-sm font-semibold text-phantom-mint hover:text-white">
          ← {{ t('blog.title') }}
        </Link>
      </div>
